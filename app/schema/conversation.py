...
class ConversationItemResponse(BaseModel):
    ...
    completion_id: str = Field(
        description="Unique identifier for the conversation in UUID format."
    )
    title: str = Field(
    ...
    conversation_origin: Optional[str] = Field(
        default=None, description="Origin or source of the conversation, if specified."
    )
...
class ConversationResponse(BaseModel):
    ...
    items: List[ConversationItemResponse] = Field(
        description="List of conversation items representing the user's chat history."
    )
    total: int = Field(
