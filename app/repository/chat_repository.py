...
class ChatRepository:
    ...
    async def save(self, entity: ChatCompletion) -> ChatCompletion:
        ...
        """
        logger.debug(f"BEGIN: save chat completion. input data: completion_id: {entity.completion_id}")
        ...
        result = await self.find_by_id(completion_id)
        logger.debug("END: save chat completion")
        return result

    async def find(self, query: dict = {}, page: int = 1, limit: int = 10, sort: dict = {"created_date": -1}, project: dict = None) -> List[ChatCompletion]:
        """
        ...
        cursor = self.db.chat_completion.find(query, project).skip(skip).limit(limit).sort(sort)
    ...
    async def find_by_id(self, completion_id: str, project: dict = None) -> ChatCompletion:
        ...
        entity = await self.db.chat_completion.find_one({"completion_id": completion_id}, project)
    ...
    async def find_messages(self, completion_id: str) -> List[ChatMessage]:
        ...
        project = {"messages": 1}
        result = await self.find_by_id(completion_id, project)
