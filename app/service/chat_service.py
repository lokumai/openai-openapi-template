...
from app.schema.chat_schema import ChatCompletionRequest, ChatCompletionResponse, ChoiceResponse, MessageResponse
...
class ChatService:
    ...
    async def handle_chat_completion(self, request: ChatCompletionRequest) -> ChatCompletionResponse:
        ...
        result.choices = [ChoiceResponse(**{"index": 0, "message": messages[0], "finish_reason": "stop"})]
        return result
    

    async def find(self, query: dict, page: int, limit: int, sort: dict, project: dict = None) -> List[ChatCompletion]:
